...
We install the `accessr` package. If you wish to output to Word format using `accessr` then you need to install the packages `officer` and `officedown`.

```{r install_for_accessr}
...
Similarly, on Windows, macOS or linux, we can print HTML output to PDF automatically, provided that we have the `pagedown` package installed.

```{r install_for_html_print}
