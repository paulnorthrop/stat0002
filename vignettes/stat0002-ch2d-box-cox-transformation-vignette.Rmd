...
There can be benefits to working with $(y_1, \ldots, y_n)$ instead of $(x_1, \ldots, x_n)$.  The methods that we use to perform *statistical inference* (i.e. to analyse data to infer answers to questions) are often based on assumptions about the behaviour of the data.  For example, we may assume that the data have been sampled from a particular probability distribution, e.g. a normal distribution, or that the mean of one variable of interest is linearly related to the value of another variable.  These assumptions can be convenient because they lead to methods of analysis that are simple, convenient and well-understood.  For more information and examples see the Wikipedia page for Data transformation.
...
Box-Cox transformation, after the authors of a paper (@BC1964) in which this type of transformation was studied extensively. (The names of the authors are a very pleasing coincidence, see the Wikipedia page for Box and Cox.
