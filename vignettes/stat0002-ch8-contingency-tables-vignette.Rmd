...
The function `chisq.test` in the `stats` package can be used to perform the chi-squared outlined at the end of [Section 8.1.1](https://paulnorthrop.github.io/stat0002book/contingency.html#indep) of the notes. We specify `correct = FALSE` to produce the same result given in the notes, that is, we do not use the Yates's correction for continuity.
